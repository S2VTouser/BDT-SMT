...
def store_masked_loaders(train_dataset: datasets, test_dataset: datasets,
    ...
    # train_mask = np.isin(np.array(train_dataset.targets), class_order[setting.i:setting.i + setting.N_CLASSES_PER_TASK])
    #
    # test_mask = np.isin(np.array(test_dataset.targets), class_order[setting.i:setting.i + setting.N_CLASSES_PER_TASK])
    #####imagenet100
    class_order = list(range(len(np.unique(train_dataset.targets))))
