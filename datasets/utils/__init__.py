...
def store_masked_loaders(train_dataset: datasets, test_dataset: datasets,
    ...
    """
    num_classes = 10
    print(setting.i)   #tasks=6 c10   n_clases_per task 1
    class_order = np.arrange(num_classes)
    assert len(class_order) == num_classes
    #print('class order:', class_order)

    if setting.i == 0:  #T1: 0-4 T2 5,
        train_mask = np.isin(np.array(train_dataset.targets),
                             class_order[setting.i:setting.i + 5])

        test_mask = np.isin(np.array(test_dataset.targets),
                            class_order[setting.i:setting.i + 5])

    else: #i=1
        train_mask = np.isin(np.array(train_dataset.targets),
                             class_order[setting.i+5:setting.i+5 + setting.N_CLASSES_PER_TASK])

        test_mask = np.isin(np.array(test_dataset.targets),
                            class_order[setting.i+5:setting.i+5 + setting.N_CLASSES_PER_TASK])

    #####
    train_dataset.data = train_dataset.data[train_mask]  #true
    test_dataset.data = test_dataset.data[test_mask]

    train_dataset.targets = np.array(train_dataset.targets)[train_mask]
    test_dataset.targets = np.array(test_dataset.targets)[test_mask]
    print('train data target:', train_dataset.targets)
    print('test data target:', test_dataset.targets)
