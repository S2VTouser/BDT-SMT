...
class BDT(ContinualModel):
    NAME = 'bdt'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(BDT, self).__init__(backbone, loss, args, transform)
    ...
    def end_task(self):
        ...
        self.temp_omega[indices_to_zero] = 0
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if self.current_task > 0:
            if self.plastic_flag:
                self.update_working_model_variables(self.plastic_model)
            if self.stable_flag:
    ...
    def update_working_model_variables(self, ema_model):
        ...
        temp_params.mul_(alpha).add_(ema_params.data, alpha=1 - alpha)

        tensor = torch.where(self.temp_omega == 0, net_params.data, temp_params)
