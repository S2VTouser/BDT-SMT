...
class SI(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform):
        ...
        self.checkpoint = self.net.parameters().data.clone().to(self.device)
        self.big_omega = None
        self.small_omega = 0
    ...
    def end_task(self, dataset):
        ...
        if self.big_omega is None:
            ...
            self.big_omega = torch.zeros_like(self.net.parammeters()).to(self.device)

        #self.big_omega += self.small_omega / ((self.net.get_params().data - self.checkpoint)**2 + self.args.xi)
        self.big_omega += self.small_omega / ((self.net.parameters().data - self.checkpoint) ** 2 + self.args.xi)
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        self.opt.zero_grad()
        print('big_omega:', self.big_omega)
